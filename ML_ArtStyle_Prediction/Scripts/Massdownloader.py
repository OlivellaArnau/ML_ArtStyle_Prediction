...
url = "https://www.wikiart.org/en/paintings-by-style/abstract-art?select=featured#!#filterName:featured,viewType:masonry"
driver.get(url)
...
if not os.path.exists('images_abstract'):
    os.makedirs('images_abstract')
    ...
    if src:
        try:
            ...
            if img_file.size[0] > 1 and img_file.size[1] > 1:
                with open(f"images_abstract/imagen_{idx}.jpg", 'wb') as handler:
